      ...
      <header className="relative flex items-center border-b border-[#e5e7eb] bg-white px-5 pb-2 pt-8">
        ...
        <div className="relative z-10 ml-auto flex shrink-0 items-center gap-2">
          ...
          {isDemo && status === 'unauthenticated' ? (
            <button
              type="button"
              onClick={() => signInFromDemoHeader()}
              className="flex max-w-[calc(100vw-10rem)] shrink-0 items-center gap-1.5 rounded-lg border border-zinc-300 bg-white px-2.5 py-1.5 text-left shadow-[0_1px_2px_rgba(0,0,0,0.08)] transition-[box-shadow,background-color,transform] hover:bg-zinc-50 hover:shadow active:scale-[0.98] focus-visible:outline focus-visible:ring-2 focus-visible:ring-[#4285F4] focus-visible:ring-offset-1"
            ...
            >
              ...
              <span className="whitespace-nowrap text-[10px] font-semibold leading-none text-[#3c4043] sm:text-[11px]">
                Sign in with Google
              </span>
            </button>
          ) : (
                        ...
                        (isTryWalkthroughPreview ? (
                          <div className="mt-2.5 space-y-2.5">
                            {(recordDisplayResult.additionalSteps || []).map((s, i) => {
                              ...
                              const supportingTitle = buildSupportingDisplayTitle({
                                action: s.action,
                              ...
                              return (
                                <div key={sid}>
                                  <div className="rounded-2xl border border-[#e5e7eb] bg-[#fafafa] px-3.5 py-2.5 text-center ring-1 ring-zinc-300/50 shadow-[0_2px_10px_rgba(0,0,0,0.04)]">
                                    <p className="mb-1 text-[8px] font-semibold uppercase tracking-[0.22em] text-[#6b7280]">
                                      {i === 0 ? 'OTHER ACTION' : 'ALSO'}
                                    </p>
                                    <p className="text-[15px] font-bold leading-[1.25] tracking-[-0.02em] text-[#111111] antialiased">
                                      {supportingTitle}
                                    </p>
                                  </div>
                                  <button
                                    ...
                                    disabled={isTryWalkthroughPreview || !!supportingAdded[sid]}
                                    title={isTryWalkthroughPreview ? 'Sign in to save and sync' : undefined}
                                    className={`group mt-2 inline-flex w-full select-none items-center justify-center gap-1.5 rounded-xl py-2.5 pl-3 pr-3 text-[13px] font-bold leading-none antialiased transition-[transform,box-shadow,filter] duration-200 ease-out ${
                                      isTryWalkthroughPreview
                                        ? 'cursor-not-allowed bg-zinc-200 text-zinc-500 shadow-none'
                                        : supportingAdded[sid]
                                          ? 'cursor-default bg-emerald-600 text-white shadow-[0_4px_18px_-4px_rgba(5,150,105,0.35),0_2px_8px_rgba(5,150,105,0.2)]'
                                          : 'text-white shadow-[0_4px_18px_-4px_rgba(79,70,229,0.3),0_2px_8px_rgba(79,70,229,0.15)] hover:shadow-[0_6px_22px_-4px_rgba(79,70,229,0.35)] hover:brightness-[1.02] active:translate-y-px active:scale-[0.982] active:brightness-[0.95]'
                                    }`}
                                    style={
                                      isTryWalkthroughPreview || supportingAdded[sid]
                                        ? undefined
                                        : { backgroundColor: '#4F46E5' }
                                    }
                                  >
                                    {supportingAdded[sid] ? (
                                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="block h-3.5 w-3.5 shrink-0 opacity-[0.95]" aria-hidden>
                                        <path d="M20 6L9 17l-5-5" strokeLinecap="round" strokeLinejoin="round" />
                                      </svg>
                                    ) : (
                                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="block h-3.5 w-3.5 shrink-0 opacity-[0.95]" aria-hidden>
                                        <rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
                                      </svg>
                                    )}
                                    <span className="tracking-tight">{supportingAdded[sid] ? 'Added' : 'Add to calendar'}</span>
                                  </button>
                                </div>
                              )
                            })}
