...
export default function VisitClarification({extraction,question,onConfirm,onSkip}: {
  ...
  const leaveUnresolved = async () => {
    ...
    try { await onSkip() } catch {
      setError(es ? 'No se pudieron guardar los cambios. Tus respuestas siguen aquí; puedes reintentar.' : 'Could not save the changes. Your answers are still here; you can retry.')
  ...
  return <dialog ref={dialog} onCancel={e => {e.preventDefault();void leaveUnresolved()}} aria-labelledby="visit-question-title" className="m-auto w-[min(94vw,30rem)] rounded-2xl bg-white p-5 text-gray-900 backdrop:bg-black/40">
    <form className="space-y-4" onSubmit={async e => {
      ...
      <label className="block text-sm">{es ? 'Tu respuesta' : 'Your answer'}<input autoFocus required disabled={busy} value={value} onChange={e=>setValue(e.target.value)} type={question.field === 'date' ? 'date' : question.field === 'time' ? 'time' : 'text'} className="mt-1 w-full rounded-xl border border-gray-300 p-3 text-base" /></label>
      {error && <p role="alert" className="text-sm text-red-700">{error}</p>}
      <div className="flex justify-end gap-3"><button disabled={busy} type="button" onClick={() => {void leaveUnresolved()}} className="rounded-xl px-4 py-3">{es ? 'Dejar pendiente' : 'Leave unresolved'}</button><button disabled={busy} type="submit" className="rounded-xl bg-indigo-600 px-4 py-3 text-white disabled:opacity-50">{busy ? (es ? 'Aplicando…' : 'Applying…') : (es ? 'Confirmar' : 'Confirm')}</button></div>
    </form>
  </dialog>
}
