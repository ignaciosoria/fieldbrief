    ...
    <div className="space-y-3">
      ...
      {actions.map(({action,index})=>{
        ...
        return <article key={index} className="rounded-2xl border border-zinc-200 bg-white p-4">
          ...
          <button type="button" disabled={recording} onClick={()=>calendarActionNeedsClarification(extraction,index)?onClarify(index):onCalendar(draft)} className="mt-3 w-full rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{es?'Añadir al calendario':'Add to calendar'}</button>
        </article>
      })}
    </div>
