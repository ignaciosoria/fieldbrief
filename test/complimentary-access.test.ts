...
test('quota copy is plain English and the plan endpoint uses the same authenticated entitlement', async () => {
  ...
  assert.equal((await response!.json()).error, "You've reached your free limit. Upgrade to Folup Pro to continue.")
  const page = await readFile('app/page.tsx', 'utf8')
  assert.match(page, /You've reached your free limit/)
  assert.match(page, /Upgrade to Folup Pro for unlimited notes and follow-ups\./)
  assert.doesNotMatch(page, /free AI allowance/)
  const route = await readFile('app/api/subscription/route.ts', 'utf8')
  assert.match(route, /has_unlimited_ai_access/)
  assert.match(route, /p_user_id: session.user.email.trim\(\)/)
  assert.match(route, /typeof data !== 'boolean'/)
})
